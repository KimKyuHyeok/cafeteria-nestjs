...
export class StoreService {
    ...
    async storeSignin(data: StoreInput): Promise<Token> {
        ...
        try {
            const store = await this.prisma.store.findFirst({ where: { name: data.name }})

            const passwordValid = await this.passwordService.validatePassword(data.password, store.password);
